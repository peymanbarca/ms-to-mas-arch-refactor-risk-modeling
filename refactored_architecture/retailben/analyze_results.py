...
def analyze_results(results_file):
    with open(results_file, 'r') as f:
        ...
        for step in step_results: 
            try:
                ...
                if decision_type.__contains__('interruption'):
                    interrupted_steps += 1
                if decision_type.__contains__('false_rejection_override'):
                    total_predicate_false_rejected += 1
                if decision_type.__contains__('false_acceptance_override_'):
        ...
        results = {
            ...
            'total_rollback': len(step_results) - total_accepted,
            'sigma_delta_qa': sigma_delta_qa,
            'sigma_delta_l': sigma_delta_l,
            'sigma_delta_f': sigma_delta_f,
            'overrides_steps': overrides_steps,
            'gov_requirement': overrides_steps / len(step_results) * 100 if len(step_results) > 0 else 0,
...
final_results = []
for file in os.listdir(pwd + '/results/Random'):
    if file.endswith('.json'):
        results_file = os.path.join(pwd + '/results/Random', file)
        results = analyze_results(results_file)
        print(json.dumps(results, indent=4))
        final_results.append(results)

print(len(final_results))
with open(pwd + '/results/final_results_Random.json', 'w') as f:
