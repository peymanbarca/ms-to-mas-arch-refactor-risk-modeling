...
logger = logging.getLogger("user-mention-agent.tracing")


def init_tracer(jaeger_cfg: dict) -> opentracing.Tracer:
    """
    Initialize Jaeger tracer from configuration.
    Falls back to no-op tracer if Jaeger is unavailable.
    """
    try:
        ...
        cfg = jaeger_client.Config(
            ...
            service_name=jaeger_cfg.get("service_name", "user-mention-agent"),
            validate=True,
        )
