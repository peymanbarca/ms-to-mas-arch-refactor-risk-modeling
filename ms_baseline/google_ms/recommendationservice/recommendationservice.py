"""
recommendationservice/main.py

Python / FastAPI + gRPC server – modernised port of the original
recommendation_server.py from GoogleCloudPlatform/microservices-demo.

Original startup sequence reproduced here:
─────────────────────────────────────────────────────────────────────────────
  1. logger.info("initializing recommendationservice")
  2. initStackdriverProfiling()        (DISABLE_PROFILER env gate)
  3. GrpcInstrumentorClient/Server     (OpenTelemetry gRPC auto-instrumentation)
  4. TracerProvider + OTLPSpanExporter (ENABLE_TRACING == "1" gate)
  5. catalog_addr = PRODUCT_CATALOG_SERVICE_ADDR  (raises if not set)
  6. channel = grpc.insecure_channel(catalog_addr)
  7. product_catalog_stub = ProductCatalogServiceStub(channel)
  8. server = grpc.server(ThreadPoolExecutor(max_workers=10))
  9. add_RecommendationServiceServicer_to_server(service, server)
 10. add_HealthServicer_to_server(service, server)
 11. server.add_insecure_port('[::]:' + port)
 12. server.start() + while True: time.sleep(10000)
─────────────────────────────────────────────────────────────────────────────

What changes vs. original:
  • grpc.aio (async) replaces ThreadPoolExecutor grpc.server
  • FastAPI HTTP server added on HTTP_PORT (default: PORT + 1000)
      GET  /health             – liveness probe
      GET  /ready              – readiness probe
      GET  /recommendations    – REST proxy for ListRecommendations RPC
  • ProductCatalog stub built with grpc.aio channel
  • OTEL tracing re-wired to async-compatible GrpcAioInstrumentor
  • PRODUCT_CATALOG_SERVICE_ADDR still required (same as original)
"""
...
app = make_health_app("recommendationservice")
