#!/usr/bin/env python3
"""
ComposePostService — Python port of ComposePostService.cpp

Usage:
    PYTHONPATH=gen-py python server.py [--config config/service-config.json] [--port 9100]

Original C++ dependencies (no own database — pure orchestrator):
    - UniqueIdService      (unique-id-service:9090)
    - MediaService         (media-service:9091)
    - TextService          (text-service:9095)
    - UserService          (user-service:9094)
    - PostStorageService   (post-storage-service:9096)
    - UserTimelineService  (user-timeline-service:9098)
    - RabbitMQ             (write-home-timeline-rabbitmq:5672)
    - Jaeger               — tracing
"""
...
from .handler     import ComposePostHandler
from .thrift_pool import ThriftClientPool
from .tracing     import init_tracer

# Import HomeTimelinePublisher from write-home-timeline-service.
# In a real deployment both packages are co-located or publisher.py is copied.
# For portability we inline a minimal publisher here.
import pika
...
# ---------------------------------------------------------------------------
# Inline publisher (avoids cross-service import)
# ---------------------------------------------------------------------------

class _RabbitPublisher:
    """
    Minimal RabbitMQ publisher for WriteHomeTimeline events.
    Creates a new connection per ComposePost call (pika is not thread-safe).
    For high throughput, replace with a per-thread connection pool.
    """
    ...
    def publish(
        self, req_id: int, post_id: int, user_id: int,
        timestamp: int, user_mentions_id: list, carrier: dict,
    ) -> None:
        payload = json.dumps({
            "req_id":           req_id,
            "post_id":          post_id,
        ...
        conn.close()
        logger.debug(
            "Published to %s: post_id=%d user_id=%d", self._queue, post_id, user_id
        )
...
def build_server(config: dict, port: int) -> TServer.TThreadedServer:
    ...
    pool_size   = num_workers * 2   # headroom

    # ---- 6 downstream Thrift client pools ----
    ...
    logger.info("RabbitMQ: %s:%s  queue=%s",
                mq_cfg.get("host"), mq_cfg.get("port"), queue_name)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ComposePostService — Python")
    ...
    port   = args.port or int(
        config.get("ComposePostService", {}).get("port", 9100)
    )
