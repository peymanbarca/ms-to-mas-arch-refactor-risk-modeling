...
logger = logging.getLogger("user-mention-service.tracing")


def init_tracer(jaeger_cfg: dict) -> opentracing.Tracer:
    """Return a Jaeger tracer, or the global no-op tracer on failure."""
    try:
        ...
        cfg = jaeger_client.Config(
            ...
            service_name=jaeger_cfg.get("service_name", "user-mention-service"),
            validate=True,
        )
