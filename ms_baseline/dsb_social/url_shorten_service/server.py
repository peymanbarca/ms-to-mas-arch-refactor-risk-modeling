#!/usr/bin/env python3
"""
UrlShortenService — Python port of UrlShortenService.cpp

Usage:
    PYTHONPATH=gen-py python server.py [--config config/service-config.json] [--port 9092]

Original C++ dependencies:
    - MongoDB  (url-shorten-mongodb:27017)  persistent mapping store
    - Memcached (url-shorten-memcached)     cache  [replaced with Redis]
    - Jaeger                                tracing
"""
...
def build_server(config: dict, port: int) -> TServer.TThreadedServer:
    ...
    logger.info("UrlShortenService starting on port %d  hostname=%s", port, hostname)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UrlShortenService — Python")
    parser.add_argument("--config", default=DEFAULT_CONFIG)
    parser.add_argument("--port",   type=int, default=None)
    args = parser.parse_args()

    config = load_config(args.config)
    port   = args.port or int(config.get("UrlShortenService", {}).get("port", 9092))

    server = build_server(config, port)
    logger.info("UrlShortenService ready")
    try:
        server.serve()
    except KeyboardInterrupt:
        logger.info("UrlShortenService shutting down")
